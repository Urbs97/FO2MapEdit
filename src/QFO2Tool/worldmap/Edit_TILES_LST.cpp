...
#pragma region popup
    ...
    for (int i = 0; i < handle->size; i++) {
        ...
        if ((match_buff[tile_num / 8] & 1 << shift_ctr) == 0) {
...
char* save_NEW_FRM_tiles_LST(tt_arr_handle* handle, char* game_path, export_state* state) {
    ...
    char* new_tile_LST = make_FRM_tile_LST(handle, nullptr);
...
char* check_FRM_LST_names(char* old_tiles_LST, tt_arr_handle* handle, export_state* state) {
    ...
    char* strt = old_tiles_LST; // keeps track of first letter of name on
                                // TILES.LST
...
void append_FRM_tiles_POPUP(user_info* usr_nfo, tt_arr_handle* handle, export_state* state,
                            bool auto_export) {
    // input name
    ImGui::Text("In order to get new FRMs to appear in the Fallout 2\n"
    ...
    ImGui::Text("(I plan on adding a feature to extract these from)\n"
