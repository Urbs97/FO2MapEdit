...
bool save_PNG_popup_INTERNAL(image_data* img_data, user_info* usr_info) {
    ...
    ImGui::Text("This will export as PNG format.\n"
...
bool save_FRM_SURFACE(char* save_name, image_data* img_data, user_info* usr_info,
    ...
    //     Windows (w/wide character support) wchar_t *w_save_name = io_utf8_wchar(save_name);
...
bool check_and_write_cfg_file(user_info* user_info, char* exe_path) {
    ...
    snprintf(cfg_filepath_buffer, sizeof(cfg_filepath_buffer), "%s%s", exe_path,
...
bool ImDialog_save_TILE_SURFACE(image_data* img_data, user_info* usr_info, Save_Info* sv_info,
    ...
    if (is_wmap) {
        ...
        if (!has_game_path) {
            ImGui::TextWrapped("FO2 Installation Folder not set.\n"
    ...
    if (success) {
        ...
        if (is_wmap) {
            ...
            snprintf(data_data, MAX_PATH, "%s/data/data/", usr_info->default_game_path);
...
uint8_t* blend_PAL_texture(image_data* img_data) {
    ...
    for (int i = 0; i < img_size; i++) {
        ...
        if (texture_buffer[i] != 0) {
            ...
            // blend_buffer[i] = img_data->FRM_dir->frame_data[0]->frame_start[i]; //
            // img_data->FRM_data[i];
        }
    }
    free(texture_buffer);
...
tt_arr_handle* export_TMAP_tiles_POPUP(user_info* usr_info, Surface* srfc, Rect* offset,
    ...
    ImGui::Text("Please type a default name for these tiles.\n"
    ...
    ImGui::InputText("Name\n(max 5 characters)", state->save_name, 6);
