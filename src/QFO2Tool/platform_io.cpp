...
wchar_t* io_get_err_str(DWORD err) {
    ...
    if (FormatMessage(
            ...
            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), lpMsgBuf, 1024, NULL) == 0) {
...
char* io_wchar_utf8(NATIVE_STRING_TYPE* src) {
    ...
    int count = WideCharToMultiByte(CP_UTF8, 0, src,
                                    -1, //  -1 if string is null-terminated
                                    buff, MAX_PATH, NULL, lpUsedDefaultChar);
...
wchar_t* io_utf8_wchar(const char* src) {
    ...
    int count = MultiByteToWideChar(CP_UTF8, 0, src,
                                    -1, // -1, the function processes the entire input string,
...
char* io_get_cwd() { return _getcwd(NULL, 0); }
...
int io_strncasecmp(NATIVE_STRING_TYPE* wstr1, NATIVE_STRING_TYPE* wstr2, int num_char) {
    return (CompareStringEx(LOCALE_NAME_USER_DEFAULT, LINGUISTIC_IGNORECASE, wstr1, -1, wstr2, -1,
                            NULL, NULL, NULL) -
            2
...
char* io_scan_dir(void* dir_stream) {
    ...
    int success = FindNextFile(hnd, &data);
    if (success == 0) {
        // TODO: log to file
        DWORD err = GetLastError();
...
char* io_path_check(char* file_name) { return file_name; }
