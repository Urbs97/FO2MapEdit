...
bool crop_animation_SURFACE(image_data* src, image_data* dst, Palette* pal, int algo,
                            shader_info* shaders) {
    int num_frms = 0;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        for (int j = 0; j < num_frms; j++) {
            ...
            calculate_bounding_box_SURFACE(&bounding_box, &FRM_bounding_box, dst_frame,
                                           &dst->ANM_dir[i].frame_box[j]);
    ...
    bool success =
        framebuffer_init(&dst->render_texture, &dst->framebuffer, dst->width, dst->height);
    if (!success) {
        // TODO: log out to file
        //  set_popup_warning(
