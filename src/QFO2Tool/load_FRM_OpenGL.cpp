...
bool framebuffer_init(GLuint* texture, GLuint* framebuffer, int w, int h) {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        set_popup_warning("[ERROR] framebuffer_init()\n\n"
...
bool load_FRM_to_SURFACE(const char* file, image_data* img_data, shader_info* shaders) {
    ...
    for (int i = 0; i < num_orients; i++) {
        ...
        for (int j = 0; j < num_frames; j++) {
            ...
            calculate_bounding_box(&frame_box, &FRM_bounding_box, frame_start,
                                   &anm_dir[i].frame_box[j]);
    ...
    img_data->height =
        img_data->ANM_bounding_box[this_dir].y2 - img_data->ANM_bounding_box[this_dir].y1;
...
bool load_FRM_OpenGL(const char* file_name, image_data* img_data, shader_info* shaders) {
    ...
    if (!success) {
        // TODO: log out to file
        set_popup_warning("[ERROR] load_FRM_OpenGL()\n\n"
    ...
    success = framebuffer_init(&img_data->render_texture, &img_data->framebuffer, img_data->width,
                               img_data->height);
    if (!success) {
        // TODO: log out to file
        set_popup_warning("[ERROR] load_FRM_OpenGL()\n\n"
    ...
    SURFACE_to_texture(img_data->ANM_dir[img_data->display_orient_num].frame_data[0],
