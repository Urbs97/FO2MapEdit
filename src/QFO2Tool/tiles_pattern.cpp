...
void export_TMAP_tiles_pattern(user_info* usr_info, tt_arr_handle* handle, char* save_path) {
    ...
    if (FRM_tiles_LST == nullptr) {
        if (usr_info->default_game_path[0] != '\0') {
            usr_info->game_files.FRM_TILES_LST =
                load_LST_file(usr_info->default_game_path, "/data/art/tiles/", "TILES.LST");
            // TODO: keep using load_LST_file()? or replace with
            //  load_FRM_tiles_LST(usr_info, state)?
        }
        if (FRM_tiles_LST == nullptr) {
            // TODO: log to file
            set_popup_warning("[ERROR] export_TMAP_tiles_pattern()\n\n"
...
void export_PAT_file_POPUP(user_info* usr_nfo, tt_arr_handle* handle, export_state* state,
    ...
    ImGui::Text("The first slot open for a pattern file is:\n");
    char* selected_PAT = select_PAT_name(&filenames);
    ImGui::Text("To use a different slot number, change it here.\n"
    ...
    if (save_pattern) {
        ...
        snprintf(save_path, MAX_PATH, "%s/data/proto/tiles/PATTERNS/%s", usr_nfo->default_game_path,
                 selected_PAT);
