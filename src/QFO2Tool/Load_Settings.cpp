...
//  hybrid approach - have it support a portable version and an installed
//  version. In the case of the portable version, you ship a configuration file
//  with it that enables portable mode, then when you start the app you read the
//  config file in the same directory as the exe
...
// So when you load the program it tries to find the config file in a few
// places, and marks the one where it found it as the configuration path, and
// when you save it tries to write there
...
void store_config_info(struct config_data* config, struct user_info* usr_info) {
    if (strncmp(config->key_buffer, "Default_Save_Path", sizeof(config->val_buffer)) == 0) {
        snprintf(usr_info->default_save_path, sizeof(usr_info->default_save_path), "%s",
                 config->val_buffer);
    }
    if (strncmp(config->key_buffer, "Default_Game_Path", sizeof(config->val_buffer)) == 0) {
        snprintf(usr_info->default_game_path, sizeof(usr_info->default_save_path), "%s",
                 config->val_buffer);
    }
    if (strncmp(config->key_buffer, "Default_Load_Path", sizeof(config->val_buffer)) == 0) {
        snprintf(usr_info->default_load_path, sizeof(usr_info->default_save_path), "%s",
                 config->val_buffer);
    }
    if (strncmp(config->key_buffer, "Show_Image_Stats",
                sizeof(config->val_buffer)) == 0) { // handle boolean
