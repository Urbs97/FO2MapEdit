...
const char* dat2_error_str(Dat2Error err) {
    switch (err) {
        case Dat2Error::OK:
            return "OK";
        case Dat2Error::FILE_OPEN_FAILED:
            return "failed to open file";
        case Dat2Error::FILE_READ_FAILED:
            return "failed to read file";
        case Dat2Error::FILE_TOO_SMALL:
            return "file too small to be a valid DAT2 archive";
        case Dat2Error::FILE_SIZE_MISMATCH:
            return "file size does not match DAT2 footer";
        case Dat2Error::TREE_SIZE_INVALID:
            return "tree size is invalid";
        case Dat2Error::TREE_PARSE_ERROR:
            return "failed to parse tree entry";
        case Dat2Error::ENTRY_OUT_OF_BOUNDS:
            return "entry data is out of bounds";
        case Dat2Error::DECOMPRESSION_FAILED:
            return "zlib decompression failed";
        case Dat2Error::FILE_WRITE_FAILED:
            return "failed to write file";
        case Dat2Error::COMPRESSION_FAILED:
            return "zlib compression failed";
        case Dat2Error::DIRECTORY_READ_FAILED:
...
static void set_zlib_input(z_stream& strm, const uint8_t* data, uint32_t size) {
    std::memcpy(static_cast<void*>(&strm.next_in), static_cast<const void*>(&data), sizeof(data));
    strm.avail_in = size;
}
...
Dat2Archive::Dat2Archive(Dat2Archive&& other) noexcept
    : m_file_buf(std::move(other.m_file_buf)), m_file_size(other.m_file_size),
      m_data_end(other.m_data_end), m_entries(std::move(other.m_entries)) {
    other.m_file_size = 0;
    other.m_data_end = 0;
}
...
uint32_t Dat2Archive::file_count() const { return static_cast<uint32_t>(m_entries.size()); }

const std::vector<Dat2Entry>& Dat2Archive::entries() const { return m_entries; }
...
Dat2Error Dat2Archive::parse_tree() {
    ...
    for (uint32_t i = 0; i < num_files; i++) {
        // Each entry: filename_len(4) + filename(N) + is_compressed(1) + decompressed_size(4) +
        // packed_size(4) + offset(4) = 17 + N
...
Dat2Status Dat2Archive::extract_to(const Dat2Entry& entry, uint8_t* out_buf,
